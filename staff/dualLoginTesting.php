...
if (isset($_POST['login']))
	...
	$pass = md5($_POST["password"]);
		...
		if ($staffid!='' && $pass!='')
		{
			$query = "SELECT * FROM `staff` WHERE `staffID` = '$staffid' And `password` = '$pass' ";
			...
			{
				$query = "SELECT * FROM `lab_assistance` WHERE `labAssistID` = '$staffid' And `password` = '$pass' ";
				$result = mysql_query($query) or die('Error');
				$login = mysql_fetch_array($result, MYSQL_ASSOC);
		...
		else
		{
			...
			echo "<script>window.location.href='loginTest.php';</script>";
		}
	}
?>
